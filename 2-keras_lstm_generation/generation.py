...
pattern = dataX[start]   # dataX:22364个list,每个list下面存着100个词的id
print("Seed : ")
...
for i in range(n_generation):
    x = np.reshape(pattern, (1, len(pattern)))  # (1,100)
    prediction = model.predict(x, verbose=0)[0]  # 生成的是one-hot向量
...
"""
笔记:
首先是随机选取dataX(22346个list)一个list,根据该长为100的list预测第101歌词可能是哪个词,
截取后100个词,作为输入,预测第101个词可能是哪个,周而复始...
"""